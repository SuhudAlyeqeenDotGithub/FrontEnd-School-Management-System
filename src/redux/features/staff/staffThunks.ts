...
export const getStaffProfiles = createAsyncThunk<ReturnStaffType[]>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await handleApiRequest("get", "http://localhost:5000/alyeqeenschoolapp/api/staff/profiles");
  ...
  async (staffData, { rejectWithValue }) => {
    try {
      const response = await handleApiRequest(
        "post",
        "http://localhost:5000/alyeqeenschoolapp/api/staff/profiles",
        staffData
      );
