...
export const SkillDialog = ({
  ...
  const validationPassed = () => {
    if (!skill) {
      setError("Missing Data: Please enter a skill");
      return false;
    }

    if (onCreateMode && existingData && existingData.includes(skill)) {
      setError("Duplicate Data: Skill already exists");
      return false;
    }
      ...
      <ContainerComponent style="w-[30%] h-[35%] gap-10 flex flex-col z-40 bg-backgroundColor overflow-auto">
        ...
        {openUnsavedDialog && (
          <YesNoDialog
            ...
            onNo={() => {
              const container = document.getElementById("skillDialogContainer");
              if (container) {
        ...
        <div className="flex justify-between items-center">
          <h2>{onCreateMode ? "New" : "Edit"} Skill</h2>
          <div className="flex justify-between items-center gap-5">
            <LoaderButton
              ...
              onClick={async () => {
                if (validationPassed()) {
                  setError("");
                  onSave(true, skill, onEditMode ? data : undefined);
                }
              }}
            />
