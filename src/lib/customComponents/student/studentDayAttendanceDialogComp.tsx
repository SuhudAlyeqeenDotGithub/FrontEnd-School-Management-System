...
const StudentDayAttendanceDialog = ({
  type,
  data,
  academicYears,
  levels,
  ...
  const updateMutation = tanMutateAny("put", "alyeqeenschoolapp/api/student/attendance/dayattendance");
  const createMutation = tanMutateAny("post", "alyeqeenschoolapp/api/student/attendance/dayattendance");
  ...
  const initialData = {
    ...
    attendanceCustomId: onCreateMode
      ? generateCustomId(`STDDAYATT`, true, "numeric")
      : data
      ? data.attendanceCustomId
      : "",

    courseId: onCreateMode ? "" : data ? data.courseId : "",
    studentDayAttendances: onCreateMode ? [] : data ? data.studentDayAttendances : [],
  ...
  const [studentDayAttendanceSearchValue, setStudentDayAttendanceSearchValue] = useState("");
  const [filterStudentDayAttendanceStatus, setFilterStudentDayAttendanceStatus] = useState("All");
  const [onFilterStudentDayAttendanceBackup, setOnFilterStudentDayAttendanceBackup] = useState<any>([]);
  const {
    attendanceCustomId,
    attendanceDate,
    attendanceStatus,
  ...
  useEffect(() => {
    const recommendedCourseManager = courseManagers.find(
  ...
  useEffect(() => {
    const recommendedLevelManager = levelManagers.find(
  ...
  useEffect(() => {
    let dataToFilter;
    let onFilterStudentDayAttendanceBackupLocal: any = [];
    if (onFilterStudentDayAttendanceBackup.length === 0) {
      dataToFilter = studentDayAttendances;
      onFilterStudentDayAttendanceBackupLocal = studentDayAttendances;
      setOnFilterStudentDayAttendanceBackup(studentDayAttendances);
    } else {
      dataToFilter = onFilterStudentDayAttendanceBackup;
      onFilterStudentDayAttendanceBackupLocal = onFilterStudentDayAttendanceBackup;
    }

    if (filterStudentDayAttendanceStatus === "All" && studentDayAttendanceSearchValue !== "") {
      const filteredData = dataToFilter.filter((studentDayAttendance: any) => {
        const { studentCustomId, studentId, studentFullName, attendance } = studentDayAttendance;
        const searchText = generateSearchText([studentCustomId, studentId, studentFullName, attendance]);
        return searchText.toLocaleLowerCase().includes(studentDayAttendanceSearchValue.toLocaleLowerCase());
      });

      setLocalData((prev: any) => ({ ...prev, studentDayAttendances: filteredData }));
    } else if (filterStudentDayAttendanceStatus !== "All" && studentDayAttendanceSearchValue !== "") {
      const filteredData = dataToFilter.filter((studentDayAttendance: any) => {
        const { studentCustomId, studentId, studentFullName, attendance } = studentDayAttendance;
        const searchText = generateSearchText([studentCustomId, studentId, studentFullName, attendance]);
        return (
          searchText.toLocaleLowerCase().includes(studentDayAttendanceSearchValue.toLocaleLowerCase()) &&
          studentDayAttendance.attendance === filterStudentDayAttendanceStatus
        );
      });
      setLocalData((prev: any) => ({ ...prev, studentDayAttendances: filteredData }));
    } else if (filterStudentDayAttendanceStatus !== "All" && studentDayAttendanceSearchValue === "") {
      const filteredData = dataToFilter.filter((studentDayAttendance: any) => {
        return studentDayAttendance.attendance === filterStudentDayAttendanceStatus;
      });
      setLocalData((prev: any) => ({ ...prev, studentDayAttendances: filteredData }));
    } else {
      setLocalData((prev: any) => ({ ...prev, studentDayAttendances: onFilterStudentDayAttendanceBackupLocal }));
      setOnFilterStudentDayAttendanceBackup([]);
    }
  }, [filterStudentDayAttendanceStatus, studentDayAttendanceSearchValue]);
  ...
  const generateEnrolledStudentDayAttendances = (enrolledStudents: any) => {
    ...
    setLocalData((prev: any) => ({
      ...prev,
      studentDayAttendances: mappedRelevantStudents_Attendance
    }));
  };

  const handleLoadStudents = async () => {
    setEnrolledStudentsData((prev: any) => ({ ...prev, isFetchingEnrolledStudents: true }));
    if (studentDayAttendances.length > 0) return;
    if (validationPassed()) {
      setError("");
      try {
        const response = await handleApiRequest(
          "post",
          "alyeqeenschoolapp/api/student/attendance/enrolleddayattendancestudents",
          {
        ...
        if (relevantStudents && relevantStudents.length > 0) {
          generateEnrolledStudentDayAttendances(relevantStudents);
  ...
  const handleReLoadStudents = async () => {
    ...
    if (validationPassed()) {
      ...
      try {
        ...
        if (relevantStudents && relevantStudents.length > 0) {
          const mappedRelevantStudents_Attendance = enrolledStudents.map((studentEnrollmentObj: any) => {
            const attendanceExist = studentDayAttendances.find(
  ...
  const handleCreateStudentDayAttendance = async () => {
    if (validationPassed()) {
      setError("");
      if (
        studentDayAttendances.filter((studentDayAttendance: any) => studentDayAttendance.attendance === "").length > 0
      ) {
  ...
  const handleUpdateStudentDayAttendance = async () => {
    if (validationPassed()) {
      setError("");
      if (
        studentDayAttendances.filter((studentDayAttendance: any) => studentDayAttendance.attendance === "").length > 0
  ...
  // console.log("filterStudentDayAttendanceStatus", filterStudentDayAttendanceStatus);
          ...
          <div className="flex justify-between items-center gap-5">
            {onCreateMode && (
              <LoaderButton
                ...
                onClick={handleCreateStudentDayAttendance}
              />
            )}
            {onEditMode && (
              <LoaderButton
                ...
                disabled={!unsaved}
                isLoading={createMutation.isPending}
                onClick={handleUpdateStudentDayAttendance}
              />
            )}
            {onViewMode && (
              <button
        ...
        <div className="flex flex-col gap-3 border border-borderColor px-4 py-6 rounded-md shadow-md">
          ...
          <div className="flex justify-between gap-5 items-center">
            <SearchComponent
              ...
              name="studentDayAttendanceSearchValue"
              value={studentDayAttendanceSearchValue}
              onChange={(e) => setStudentDayAttendanceSearchValue(e.target.value)}
            />{" "}
            <div className="">
              <SelectInputComponent
                title=""
                name="filterStudentDayAttendanceStatus"
                placeholder="Student Attendance Status *"
                value={filterStudentDayAttendanceStatus}
                onChange={(e) => setFilterStudentDayAttendanceStatus(e.target.value)}
                options={[
                  { value: "All", label: "All" },
            ...
            <div>
              ...
              {!isFetchingEnrolledStudent && studentDayAttendances.length > 0 && (
                <button
          ...
          <div className="flex justify-between gap-2 mt-8 mx-2">
            <div className="flex flex-col gap-1 items-center justify-center">
              ...
              <span className="text-[15px] text-foregroundColor-2 font-medium">{studentDayAttendances.length}</span>
            </div>
            <div className="flex flex-col gap-1 items-center justify-center">
              ...
              <span className="text-[15px] text-foregroundColor-2 font-medium">
                {studentDayAttendances.filter((attendance: any) => attendance.attendance === "Present").length}
              </span>
            </div>
            <div className="flex flex-col gap-1 items-center justify-center">
              ...
              <span className="text-[15px] text-foregroundColor-2 font-medium">
                {studentDayAttendances.filter((attendance: any) => attendance.attendance === "Late (Excused)").length}
              </span>
            </div>
            <div className="flex flex-col gap-1 items-center justify-center">
              ...
              <span className="text-[15px] text-foregroundColor-2 font-medium">
                {studentDayAttendances.filter((attendance: any) => attendance.attendance === "Late (Unexcused)").length}
              </span>
            </div>
            <div className="flex flex-col gap-1 items-center justify-center">
              ...
              <span className="text-[15px] text-foregroundColor-2 font-medium">
                {studentDayAttendances.filter((attendance: any) => attendance.attendance === "Absent (Excused)").length}
              </span>
            </div>
            <div className="flex flex-col gap-1 items-center justify-center">
              ...
              <span className="text-[15px] text-foregroundColor-2 font-medium">
                {
                  studentDayAttendances.filter((attendance: any) => attendance.attendance === "Absent (Unexcused)")
                    .length
                }
              </span>
            </div>
          ...
          >
            <div className="bg-backgroundColor-3 text-foregroundColor font-semibold h-14 flex justify-between items-center">
              ...
              <div
                ...
                onClick={() => {
                  ...
                  const sortedData = [...studentDayAttendances].sort((a, b) => {
                    return sortOrder === "asc"
                      ? a["studentFullName"].localeCompare(b["studentFullName"])
                      : b["studentFullName"].localeCompare(a["studentFullName"]);
                  });

                  setLocalData((prev: any) => ({ ...prev, studentDayAttendances: sortedData }));
            ...
            <div className="bg-backgroundColor">
              ...
              ) : studentDayAttendances.length < 1 && !isFetchingEnrolledStudent ? (
                <div>
                  <div className="text-center py-4">No student attendance found. Load student to get started</div>
                </div>
              ) : (
                studentDayAttendances.map((attendanceObj: any, index: any) => {
                      ...
                      <span className="w-[200px] text-center whitespace-nowrap font-medium">
                        ...
                        <SelectInputComponent
                          ...
                          onChange={(e) => {
                            setLocalData((prev: any) => ({
                              ...prev,
                              studentDayAttendances: prev.studentDayAttendances.map((studentDayAttendanceObj: any) =>
                                studentDayAttendanceObj.studentCustomId === studentCustomId
                                  ? { ...studentDayAttendanceObj, attendance: e.target.value }
                                  : studentDayAttendanceObj
                              )
                            }));

                            setOnFilterStudentDayAttendanceBackup(
                              onFilterStudentDayAttendanceBackup.map((studentDayAttendanceObj: any) => {
                                return studentDayAttendanceObj.studentCustomId === studentCustomId
                                  ? { ...studentDayAttendanceObj, attendance: e.target.value }
                                  : studentDayAttendanceObj;
                              })
                            );
...
export default StudentDayAttendanceDialog;
